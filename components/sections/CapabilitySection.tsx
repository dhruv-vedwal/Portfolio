...
export default function CapabilitySection() {
  return (
    <section className="py-20 border-t border-border bg-transparent relative overflow-hidden">
      ...
      <div className="absolute bottom-1/4 left-1/2 -translate-x-1/2 w-[600px] h-[300px] bg-primary-glow blur-[140px] rounded-full opacity-15 pointer-events-none" />
          ...
          {CAPABILITIES.map((cap, idx) => {
            ...
            return (
              <FadeIn key={idx} direction="up" delay={idx * 0.08}>
                <Card className="h-full group relative p-8 flex flex-col justify-between hover:border-primary/40 hover:shadow-[0_0_20px_var(--primary-glow)] transition-all duration-300">
                  {/* Visual accent */}
                  ...
                  <div className="flex flex-col gap-5">
                    ...
                    <div className="flex items-start gap-4">
                      <div className="w-12 h-12 rounded-xl bg-card-elevated border border-border flex items-center justify-center group-hover:border-primary/40 group-hover:text-primary transition-all duration-300">
                      ...
                      <div>
                        ...
                        <CardTitle className="text-xl font-bold font-sans text-foreground group-hover:text-foreground/95 transition-colors">
                          {cap.title}
                        </CardTitle>
                      </div>
                    </div>
                  ...
                  <div className="mt-8 border-t border-border/30 pt-5 flex flex-wrap gap-1.5">
                    {cap.technologies.map((tech, techIdx) => (
                      <span
                        key={techIdx}
                        className="px-2.5 py-1 rounded bg-card-elevated/70 border border-border/80 text-[10px] text-foreground font-mono font-medium hover:border-primary/20 hover:bg-primary-glow/10 transition-colors"
                      >
                        {tech}
                      </span>
                    ))}
